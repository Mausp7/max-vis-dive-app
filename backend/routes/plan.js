...
const Dive = require('../logic/Dive');

const dive = new Dive();

// POST returns the returnvalue of dive.noDecompressionDive(maxDepth, steps, gasO2, gradientFactor)
router.post('/table', auth({block: false}), async (req, res) => {
    ...
    const table = dive.noDecompressionDive(Number(req.body.maxDepth), Number(req.body.steps), req.body.gas, Number(req.body.gradientFactor));
    res.json(table);
});

// POST returns the returnvalue of dive.dive(dives, gases, descentSpeed, ascentSpeed)
router.post('/dive', auth({block: true}), async (req, res) => {
    ...
    const divePlan = dive.dive(req.body.dives, req.body.gases, req.body.descentSpeed, req.body.ascentSpeed, req.body.gasConsumption, req.body.gradFLow, req.body.gradFHigh);
    res.json(divePlan);
});
